...
#from utils.rsna_eval import evaluate
from utils.map_v2 import evaluation
...
def get_val_roidb(dataset_name, proposal_file):
...
def eval_net(model, dataset_names, gt_file, submit_threshold=0.4, output_dir=None, proposal_files=cfg.TRAIN.PROPOSAL_FILES):
    ...
    roidbs = [get_val_roidb(*args) for args in zip(dataset_names, proposal_files)]
    ...
    submission_dict = {}
    for i, entry in tqdm(enumerate(roidb), total=len(roidb),
                                               desc='Validation'):
        predictionstring = ''
        im = cv2.imread(entry['image'])
        assert im is not None
        timers = defaultdict(Timer)
        cls_boxes, cls_segms, cls_keyps = im_detect_all(model=model, im=im, timers=timers)
        im_name, _ = os.path.splitext(os.path.basename(entry['image']))
        ...
        for box in boxes:
            ...
            width = bbox[2] - bbox[0]
            predictionstring += str(score) + ' ' + str(x) + ' ' + str(y) + ' ' + str(width) + ' ' + str(height) + ' '
        submission_dict[im_name] = predictionstring

    assert len(submission_dict) <= len(roidb)

    sub = pd.DataFrame.from_dict(submission_dict,orient='index')
    sub.index.name = 'patientId'
    sub.columns = ['PredictionString']
    sub.to_csv(os.path.join(output_dir,"submission.csv"))

    #performance = evaluate(gtfile = gt_file, predfile=os.path.join(output_dir,"submission.csv"))
    
    performance = evaluation(gt_file, os.path.join(output_dir,"submission.csv"))
