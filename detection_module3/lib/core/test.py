...
def im_detect_all_propo(model, im, box_proposals=None, timers=None):
    """Process the outputs of model for testing
    Args:
      model: the network module
      im_data: Pytorch variable. Input batch to the model.
    ...
    timers['im_detect_bbox'].tic()
    if cfg.TEST.BBOX_AUG.ENABLED:
        print("box augmented true______________________________________")
        scores, boxes, im_scale, blob_conv = im_detect_bbox_aug(
            model, im, box_proposals)
    else:
        print("box augmented false__________________________________________")
        #print("box proposals size in im_detect_all_propo ", box_proposals.shape)
        scores, boxes, im_scale, blob_conv = im_detect_bbox(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    timers['im_detect_bbox'].toc()
    ...
    print("socres size ", scores.shape)
    cls_boxes = np.hstack((cls_boxes,scores))

    return cls_boxes, None, None

def im_detect_all_propo_result(model, im, box_proposals=None, timers=None):
    """Process the outputs of model for testing
    Args:
      model: the network module
      im_data: Pytorch variable. Input batch to the model.
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        print("box augmented false__________________________________________")
        #print("box proposals size in im_detect_all_propo ", box_proposals.shape)
        scores, boxes, im_scale, blob_conv = im_detect_bbox_propo_result(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    timers['im_detect_bbox'].toc()
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
   ...
   # print("input shape in im_detect_bbox ", len(inputs['rois']))
    if cfg.DEDUP_BOXES > 0 and not cfg.MODEL.FASTER_RCNN:
#        print("DEDUP BOXES ??????  yes__________________________________")
...
#    print("im  ",im)
#    print("boxes ",boxes)
   # print("input shape in im_detect_bbox2 ", len(inputs['rois']))
    ...
    #print("input shape in im_detect_bbox3 ", len(inputs['rois']))
...
#    print("return_dict['bbox_pred'] shape in im_detect_bbox4 ", len(return_dict['bbox_pred']))
    # cls prob (activations after softmax)
    scores = return_dict['cls_score'].data.cpu().numpy().squeeze()
    # In case there is 1 proposal
    scores = scores.reshape([-1, scores.shape[-1]])
#    print("scres shape in im_detect_bbox5 ", scores.shape)
...
#    print("pred_boxes shape in im_detect_bbox6 ", pred_boxes.shape)

    if cfg.DEDUP_BOXES > 0 and not cfg.MODEL.FASTER_RCNN:
        # Map scores and predictions back to the original set of boxes
        scores = scores[inv_index, :]
        pred_boxes = pred_boxes[inv_index, :]
#    print("boxes shape in im_detect_box end ", pred_boxes.shape )
    return scores, pred_boxes, im_scale, return_dict['blob_conv']


def im_detect_bbox_propo_result(model, im, target_scale, target_max_size, boxes=None):
    """Prepare the bbox for testing"""
