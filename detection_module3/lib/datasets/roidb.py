...
def aug_for_training(roidb):
    filtered_roidb = []
    rm_hard_num = 0
    for entry in roidb:
        big_or_small = entry['big_or_small']
        hard_or_easy = entry['hard_or_easy']
    ...
    num = len(roidb)
    num_after = len(filtered_roidb)
    logger.info('aug: filteted {} hard roidbs and multiply small roidbs 3 times: final roidb entries: {} -> {}'.
                format(rm_hard_num, num, num_after))
    return filtered_roidb
