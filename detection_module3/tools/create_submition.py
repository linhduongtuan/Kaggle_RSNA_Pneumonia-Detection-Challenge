...
def parse_args():
    ...
    parser.add_argument(
        '--submit_threshold',
        help='the threshold of wrirting the bbox to the csv',
...
def main():
    ...
    # proposals_boxes, proposals_scores, proposals_img_names = create_proposal('./proposals/proposals_test.pkl')
    ...
    for i in xrange(num_images):
        ...
        #        for p_ids in range(len(proposals_img_names)):
        #            if proposals_img_names[p_ids] == imglist[i]:
        #                break
        #        proposal_b = proposals_boxes[p_ids]

        timers = defaultdict(Timer)
        # print(len(proposal_b))
        ...
        for box in boxes:
            ...
            predictionstring += '%.2f %d %d %d %d ' % (score, int(x), int(y), int(width), int(height))
