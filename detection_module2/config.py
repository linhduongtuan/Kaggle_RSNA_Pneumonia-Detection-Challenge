...
# dataset -----------------------
_C.DATA.BASEDIR = 'path/to/data_prepare/'
_C.DATA.IMGDIR = 'images'  # path to image dir
_C.DATA.TRAIN = ['train_pos', 'train_neg', ]  # i.e., trainval35k
_C.DATA.NUM_CATEGORY = 1
_C.DATA.CLASS_NAMES = ['bg', 'Pneumonia']  # NUM_CLASS (NUM_CATEGORY+1) strings, to be populated later by data loader. The first is BG.
...
_C.TRAIN.BASE_LR = 1e-3  # defined for a total batch size of 8. Otherwise it will be adjusted automatically
_C.TRAIN.WARMUP = 1000  # in terms of iterations. This is not affected by #GPUs
_C.TRAIN.STEPS_PER_EPOCH = 800
...
C.TRAIN.LR_SCHEDULE = [120000, 160000]  # "2x" schedule in detectron
_C.TRAIN.NUM_EVALS = 31  # number of evaluations to run during training
...
# _C.RPN.ANCHOR_RATIOS = (0.25, 0.5, 1., 2.,4.)
...
_C.FRCNN.BG_THRESH = 0.5
_C.FRCNN.FG_RATIO = 0.5  # fg ratio in a ROI batch
...
_C.TEST.RESULT_SCORE_THRESH = 0.
_C.TEST.RESULT_SCORE_THRESH_VIS = 0.  # only visualize confident results
_C.TEST.RESULTS_PER_IM = 100000
