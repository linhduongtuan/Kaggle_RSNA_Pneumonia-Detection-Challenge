...
from model_rpn import rpn_losses_iou, generate_rpn_proposals, generate_rpn_proposals_iou
...
def multilevel_roi_align(features, rcnn_boxes, resolution):
    ...
    level_id_perm = tf.to_int32(tf.concat(level_ids, axis=0))  # A permutation of 1~N
    level_id_invert_perm = tf.invert_permutation(level_id_perm)
    all_rois = tf.gather(all_rois, level_id_invert_perm)
    return all_rois


def multilevel_rpn_losses_iou(
    ...
    with tf.name_scope('rpn_losses'):
        for lvl in range(num_lvl):
            ...
            label_loss, box_loss, iou_loss = rpn_losses_iou(
                anchors.gt_labels, anchors.encoded_gt_boxes(), gt_boxes,
                anchors.decode_logits(multilevel_box_logits[lvl]),
                multilevel_label_logits[lvl], multilevel_box_logits[lvl], multilevel_iou_logits[lvl],
                name_scope='level{}'.format(lvl + 2))
            losses.extend([label_loss, box_loss, iou_loss])

        total_label_loss = tf.add_n(losses[::3], name='label_loss')
        total_box_loss = tf.add_n(losses[1::3], name='box_loss')
        total_iou_loss = tf.add_n(losses[2::3], name='iou_loss')
        add_moving_summary(total_label_loss, total_box_loss, total_iou_loss)
    return total_label_loss, total_box_loss, total_iou_loss
...
@under_name_scope()
def generate_fpn_proposals_iou(
        multilevel_anchors, multilevel_label_logits,
    ...
    assert len(multilevel_label_logits) == num_lvl
    assert len(multilevel_box_logits) == num_lvl
    assert len(multilevel_iou_logits) == num_lvl
    ...
    if cfg.FPN.PROPOSAL_MODE == 'Level':
        ...
        for lvl in range(num_lvl):
            with tf.name_scope('Lvl{}'.format(lvl + 2)):
                ...
                proposal_boxes, proposal_scores, proposal_ious = generate_rpn_proposals_iou(
                    tf.reshape(pred_boxes_decoded, [-1, 4]),
                    tf.reshape(multilevel_label_logits[lvl], [-1]),
                    tf.reshape(multilevel_iou_logits[lvl], [-1]),
                    image_shape2d, fpn_nms_topk)
                all_boxes.append(proposal_boxes)
                all_scores.append(proposal_scores)
                all_ious.append(proposal_ious)

        proposal_boxes = tf.concat(all_boxes, axis=0)  # nx4
        proposal_scores = tf.concat(all_scores, axis=0)  # n
        proposal_ious = tf.concat(all_ious, axis=0)  # n

        a, b = 1., 1.
        scores_ious = proposal_scores# a * tf.sigmoid(proposal_scores) + b * tf.sigmoid(proposal_ious)

        proposal_topk = tf.minimum(tf.size(proposal_scores), fpn_nms_topk)
        proposal_scores_ious, topk_indices = tf.nn.top_k(scores_ious, k=proposal_topk, sorted=False)
        proposal_boxes = tf.gather(proposal_boxes, topk_indices)
        proposal_scores = tf.gather(proposal_scores, topk_indices)
        proposal_ious = tf.gather(proposal_ious, topk_indices)

        # probs = tf.sigmoid(proposal_scores)
        # topk_indices = tf.reshape(tf.where(probs >= 0.25), [-1])
        # proposal_boxes = tf.gather(proposal_boxes, topk_indices)
        # proposal_scores = tf.gather(proposal_scores, topk_indices)
        # proposal_ious = tf.gather(proposal_ious, topk_indices)
        ...
        for lvl in range(num_lvl):
            with tf.name_scope('Lvl{}'.format(lvl + 2)):
                ...
                all_boxes.append(tf.reshape(pred_boxes_decoded, [-1, 4]))
                all_scores.append(tf.reshape(multilevel_label_logits[lvl], [-1]))
                all_ious.append(tf.reshape(multilevel_iou_logits[lvl], [-1]))
        all_boxes = tf.concat(all_boxes, axis=0)
        all_scores = tf.concat(all_scores, axis=0)
        all_ious = tf.concat(all_ious, axis=0)
        proposal_boxes, proposal_scores, proposal_ious = generate_rpn_proposals_iou(
            all_boxes, all_scores, all_ious, image_shape2d,
            cfg.RPN.TRAIN_PRE_NMS_TOPK if ctx.is_training else cfg.RPN.TEST_PRE_NMS_TOPK,
            cfg.RPN.TRAIN_POST_NMS_TOPK if ctx.is_training else cfg.RPN.TEST_POST_NMS_TOPK)
