...
def get_anchor_labels(anchors, gt_boxes, crowd_boxes, gt_labels):
    """
    ...
    # if img don't have any ground truth, return all anchor_labels is 0
    if np.max(gt_labels) == 0:
        anchor_boxes = np.zeros((NA, 4), dtype='float32')
        anchor_labels = np.zeros((NA,), dtype='int32')  # NA,
        target_num_bg = cfg.RPN.BATCH_PER_IM
...
def get_rpn_anchor_input(im, boxes, is_crowd, klass):
    ...
    anchor_labels, anchor_gt_boxes = get_anchor_labels(inside_anchors, boxes[is_crowd == 0],
                                                                        boxes[is_crowd == 1],
                                                                        klass[is_crowd == 0])
...
def get_train_dataflow():
    ...
    # Aug data
    # auglbl_8 = [4, 7, 8, 9, 10, 11, 12, 13, 14, 15]
    # auglbl_2 = [3, 5, 6]
    # roidbs2 = []
    # for roidb in roidbs:
    #    for cls in roidb['class']:
    #        if cls in auglbl_2:
    #            roidbs2.append(roidb)
    #        if cls in auglbl_8:
    #            for ai in range(2):
    #                roidbs2.append(roidb)
    # logger.info("aug images: {}-->{}".format(len(roidbs),len(roidbs) + len(roidbs2)))
    # for roidb in roidbs2:
    #    roidbs.append(roidb)
    ...
    def preprocess(roidb):
        fname, boxes, klass, is_crowd, imgClass = roidb['file_name'], roidb['boxes'], roidb['class'], roidb['is_crowd'], \
                                                  roidb['imgClass']
        ...
        ret = {'image': im}
        ret['gt_img_label'] = np.array(imgClass, dtype=int).reshape(-1)
        # rpn anchor:
        try:
            if cfg.MODE_FPN:
