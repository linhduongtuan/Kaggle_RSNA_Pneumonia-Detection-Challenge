...
from utils.box_refine import soft_nms_py, box_voting


DetectionResult = namedtuple(
    'DetectionResult',
    ['box', 'score', 'class_id', 'iou', 'mask'])
    #['box', 'score', 'class_id', 'img_level_lbl', 'img_level_lbl_score','mask'])
"""
box: 4 float
score: float
...
def detect_one_image(img, model_func):
    ...
    boxes, probs, labels, ious, img_level_label, img_level_label_score, *masks = model_func(resized_img)
    boxes = boxes / scale
    ...
    results = [DetectionResult(*args) for args in zip(boxes, probs, labels, ious, masks)]
    return results, img_level_label
