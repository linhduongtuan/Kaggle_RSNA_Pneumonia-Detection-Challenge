...
def csv2json_bg(label_csv):
    ...
    for Dataset in all_data:
        ...
        if Dataset == 'train':
            print('train imgs:', 20000, 'total_bboxes:', j)
        if Dataset == 'val':
            print('val imgs:', ii - 20000, 'total_bboxes:', j)

        folder = './annotations'
        if not os.path.exists(folder):
            os.makedirs(folder)
        json_name = '{}/{}_neg.json'.format(folder, Dataset)
